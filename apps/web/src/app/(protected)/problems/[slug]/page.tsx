...
export default async function ProblemPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const problem = (categories.problems as Array<{ slug: string; name: string }>).find((p) => p.slug === slug);
  const { results, filtered_total } = await resourcesForTaxonomy(pool(), "problem", slug, 24, 0);

  return (
    <div className="mx-auto max-w-7xl px-4 py-10 md:px-6">
      <h1 className="text-2xl font-medium">{problem?.name ?? slug.replace(/-/g, " ")}</h1>
