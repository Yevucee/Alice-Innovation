...
# environment. Sourced by install.sh and start.sh. Each helper is a no-op when
# the resource it manages is already present, so it is safe to call from both
# the one-time install phase and the per-boot start phase. This keeps the
# environment working whether it runs just-in-time (install always runs) or from
# a prebuilt image (install is skipped on boot, so start must self-heal).
